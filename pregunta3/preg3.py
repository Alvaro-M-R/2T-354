...
#1) Las reglas del árbol de decisión. 2) El número total de reglas.
# librerias
import pandas as pd
import numpy  as np
...
# Funcion: retorna verdad si una columna es numerica
...
# Función: devuelve verdadero, si un valor es numérico
...
# Función: realiza una prueba Chi_cuadrado o una prueba exacta de Fisher           
...
def prediction_dt_c45(model, Xdata):
    ...
    # Preprocesamiento: valores booleanos
...
# Función: Calcula la relación de ganancia de información  
...
# Función: división binaria en variables continuas
...
# Función: Algoritmo C4.5
def dt_c45(Xdata, ydata, cat_missing = "none", num_missing = "none", pre_pruning = "none", chi_lim = 0.1, min_lim = 5):
    
    ################     Parte 1 - Preprocesamiento    #############################
    # Preprocesamiento: creación de estructuras de datos
    ydata = ydata.apply(str) # La variable de destino debe ser una cadena
    name  = ydata.name
    if (name is None):
        name  = 'class'
    ...
    # Preprocesamiento: valores perdidos
    if cat_missing != "none":
    ...
    # Preprocesamiento - Lista de palabras únicas
    unique = []
    uniqueWords = []
    for j in range(0, dataset.shape[1]): 
        for i in range(0, dataset.shape[0]):
...
print(dt_model)
# prediccion
test =  df.iloc[0:1, 0:2]
#print(prediction_dt_c45(dt_model, test))
y_pred=prediction_dt_c45(dt_model, test)
print(y_pred.head(n = 5))
###############################################################
